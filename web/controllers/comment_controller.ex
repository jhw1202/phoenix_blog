defmodule Pxblog.CommentController do
  ...
  def create(conn, %{"comment" => comment_params, "post_id" => post_id}) do
    ...
    case Repo.insert(changeset) do
      {:ok, _comment} ->
        conn
        |> put_flash(:info, "Successfully added comment")
        |> redirect(to: user_post_path(conn, :show, post.user, post))
      {:error, changeset} ->
        render(conn, Pxblog.PostView, "show.html", post: post, user: post.user,
  ...
  defp set_post(conn) do
    post =
      Repo.get!(Post, conn.params["post_id"])
      |> Repo.preload(:user)
  ...
  defp set_post_and_authorize_user(conn, _opts) do
    ...
    if is_authorized_user?(conn) do
      ...
      conn
      |> put_flash(:error, "You are not authorized to modify this comment")
